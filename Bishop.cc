...
void Bishop::attachToCells(Board& b) {
    ...
    int r = getRow() - 1;
    int c = getCol() - 1; // now r and c are a diagonal top - left to the piece
    while (r >= 0 && c >= 0) {
        if (!b.getCellAt(r,c).getPiece()) {
            b.getCellAt(r,c).attach(this);
            r--; 
    ...
    int r = getRow() + 1; 
    int c = getCol() - 1; // now r and c are a diagonal bottom - left to the piece
    while (r < 8 && c >= 0) {
        if (!b.getCellAt(r,c).getPiece()) {
            b.getCellAt(r,c).attach(this);
            r++; 
    ...
    int r = getRow() - 1; 
    int c = getCol() + 1; // now r and c are a diagonal top - right to the piece
    while (r >= 0 && c < 8) {
        if (!b.getCellAt(r,c).getPiece()) {
            b.getCellAt(r,c).attach(this);
            r--; 
